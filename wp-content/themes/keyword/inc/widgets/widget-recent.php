 ...
 * @since      1.0.0
 */
class keyword_Recent_Widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_options = array(
			'classname'   => 'widget-keyword-recent widget_posts_thumbnail',
			'description' => __( 'Display recent posts with thumbnails.', 'keyword' )
		);

		// Create the widget.
		parent::__construct(
			'keyword-recent',                                   // $this->id_base
			__( '&raquo; Recent Posts', 'keyword' ), // $this->name
	...
	function widget( $args, $instance ) {
		...
		if ( false === ( $recent = get_transient( 'keyword_recent_widget_' . $this->id ) ) ) {
			...
			// Store the transient.
			set_transient( 'keyword_recent_widget_' . $this->id, $recent );
	...
	function flush_widget_transient() {
		delete_transient( 'keyword_recent_widget_' . $this->id );
	}
	...
	function form( $instance ) {
		...
		// Default value.
		$defaults = array(
			'title'     => esc_html__( 'Recent Posts', 'keyword' ),
			'limit'     => 5,
			'show_date' => true
		);
